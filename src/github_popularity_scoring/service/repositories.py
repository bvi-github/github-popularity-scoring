...
from github_popularity_scoring.domain.entities import Repository, RepositorySearchCriteria, ScoredRepository
...
class SearchRepositoriesUseCase:
    ...
    def __init__(self, repository_search: RepositorySearchPort, scorer: PopularityScorer) -> None:
        self._repository_search = repository_search
        self._scorer = scorer

    async def execute(self, criteria: RepositorySearchCriteria) -> list[ScoredRepository]:
        language = criteria.language

        if not language:
            raise ValueError("Language not provided")
        if criteria.created_after > date.today():
            raise ValueError("created_after must not be in future")
        if criteria.limit < 1:
        ...
        return sorted(scored_repositories, key=lambda repo: repo.popularity_score, reverse=True)[:criteria.limit]
